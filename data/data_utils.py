...
class PointcloudRotate(object):
    def __call__(self, pc):
        bsize = pc.size()[0]
        for i in range(bsize):
            rotation_angle = (np.random.uniform()-0.5) * 2 * np.pi * (30/360)
...
class PointcloudRandomPartDropout(object):
    ...
    def __call__(self, pc, label):
        ...
        for i in range(bsize):
            ...
            cur_label = cur_label.cpu().numpy()
            cur_label = cur_label - np.min(cur_label)
            drop_label = i % (np.max(cur_label)+1)
            drop_idx = np.where(cur_label == drop_label)[0]
...
class PointcloudRandomPositionDropout(object):
    ...
    def __call__(self, pc, label):
        ...
        for i in range(bsize):
            ...
            if  i//3 == 0:
                pc_temp = cur_pc.clone().cpu().numpy()[:,0]
            elif i//3 == 1:
                pc_temp = cur_pc.clone().cpu().numpy()[:,1]
            elif i//3 == 2:
                pc_temp = cur_pc.clone().cpu().numpy()[:,2]

            l = np.max(pc_temp)-np.min(pc_temp)
            if  i%3 == 0:
                drop_idx = np.where( pc_temp-np.min(pc_temp) < l/3 )[0]
            elif i%3 == 1:
                drop_idx = np.where( pc_temp-np.min(pc_temp) > 2*l/3 )[0]
            elif i%3 == 2:
                drop_idx = np.where( (pc_temp-np.min(pc_temp) < 2*l/3) & (pc_temp-np.min(pc_temp) > l/3))[0]
