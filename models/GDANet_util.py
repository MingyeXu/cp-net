...
def knn(x, k):
    inner = -2*torch.bmm(x.transpose(2, 1).contiguous(), x)
...
def GDM(x, points, M):
    ...
    A = torch.matmul(D.cpu(), w.cpu()).cuda()  # normalized adjacency matrix A_hat
    ...
    xss = points.data.clone().view(batch_size * num_points, -1)
    xss[indices,:]=xss[0,:]
    xgg = points.data.clone().view(batch_size * num_points, -1)
    xgg[indiceg,:]=xgg[0,:]

    xs = points.data.clone().view(batch_size * num_points, -1)
    t_s = xs[indices,:].clone()
    xs_jittered_data = t_s.new(t_s.size(0), 3).normal_(
        mean=0.0, std=STD
    ).clamp_(-CLIP,CLIP)
    xs[indices,:]+=xs_jittered_data

    xg = points.data.clone().view(batch_size * num_points, -1)
    t_g = xg[indices,:].clone()
    xg_jittered_data = t_g.new(t_g.size(0), 3).normal_(
        mean=0.0, std=STD
    ).clamp_(-CLIP,CLIP)
    
    xg[indiceg,:]+=xg_jittered_data

    xs = xs.view(batch_size, num_points, -1)  # b,M,c
    xg = xg.view(batch_size, num_points, -1)  # b,M,c

    return xs, xg,xss,xgg
