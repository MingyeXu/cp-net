...
def CoSeg_function(feats, K, max_iter=50):
    '''
    feats: [B, C, N]
    K : int
    '''
    ...
    with torch.no_grad():
        ...
        flat_features = feats.permute(0, 2, 1).contiguous().view(-1, feats.size(1))
        W, _ = NMF(flat_features, K, random_seed=1, cuda=True, max_iter=max_iter, verbose=False)
        # print(torch.isnan(W))
        ...
        while isnan > 0:
            ...
            W, _ = NMF(flat_features, K, random_seed=1, cuda=True, max_iter=max_iter, verbose=False)
            isnan = torch.sum(torch.isnan(W).float())
        heatmap = W.view(feats.size(0), feats.size(2), K).permute(0,2,1).contiguous() #[B, K,N]
    return heatmap
